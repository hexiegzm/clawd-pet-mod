...
// ── File Feed Mod ──────────────────────────────────────────
// Drag files onto Clawd → shy animation → drop → happy animation → Claude Code
var _fileFeedActive = false;
...
if (!isMac) {
  area.addEventListener("dragover", (e) => {
    ...
    // Show shy "ooh gimme!" animation
    if (!_fileFeedActive && !isDragReacting && !isReacting) {
      _fileFeedActive = true;
      window.hitAPI.playClickReaction("clawd-aegyo-shy.svg", 60000);
    }
  });

  area.addEventListener("dragleave", (e) => {
    // Only reset if we actually left the hit area (not entering a child)
    if (!_fileFeedActive) return;
    if (e.relatedTarget && area.contains(e.relatedTarget)) return;
    _fileFeedActive = false;
    // End the reaction so Clawd reverts to idle
    window.hitAPI.endDragReaction();
  });
  ...
  area.addEventListener("drop", (e) => {
    ...
    // Show happy "yum!" notification animation
    window.hitAPI.playClickReaction("clawd-notification.svg", 2500);
  ...
  window.hitAPI.onDropAccepted(() => {
    // Notification animation is already playing from the drop handler.
    // No extra reaction needed — the modal is playing clawd-notification.svg.
  });
}
